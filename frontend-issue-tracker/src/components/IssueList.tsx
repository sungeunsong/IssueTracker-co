...
import type { Issue, User, StatusItem, Project } from "../types";
...
export const IssueList: React.FC<IssueListProps> = ({
  ...
  const projectStatuses = project?.statuses || DEFAULT_STATUSES;
  const projectTypes = project?.types || DEFAULT_ISSUE_TYPES;
      ...
      >
        <div className="hidden sm:block">
          <p className="text-sm text-slate-700">
            ...
            {" - "}
            <span className="font-medium">
              {Math.min(currentPage * itemsPerPage, totalIssues)}
            </span>{" "}
            표시 중
          </p>
        </div>
          ...
          <div className="hidden sm:flex sm:items-center sm:space-x-1 mx-2">
            {startPage > 1 && (
              <>
                <button
                  ...
                  className="relative inline-flex items-center px-2.5 py-1.5 border border-slate-300 bg-white text-xs font-medium text-slate-500 hover:bg-slate-50 rounded-md"
                >
                  1
                </button>
                {startPage > 2 && (
                  <span className="px-1.5 py-1.5 text-xs text-slate-500">
                    ...
                  </span>
                )}
              </>
            ...
            {pageNumbers.map((num) => (
              <button
                ...
                className={`relative inline-flex items-center px-2.5 py-1.5 border text-xs font-medium rounded-md ${
                  currentPage === num
  ...
  if (totalIssues === 0 && issues.length === 0) {
    return (
      <div className="text-center py-10 px-6">
        ...
        <h3 className="mt-2 text-base font-medium text-slate-800">
          표시할 이슈가 없습니다.
        </h3>
        <p className="mt-1 text-sm text-slate-500">
          검색 조건을 변경하거나 새 이슈를 등록해주세요.
        </p>
      </div>
    );
  }
      ...
      <table className="min-w-full divide-y divide-slate-200">
        <thead className="bg-slate-100">
          <tr>
            <th
              scope="col"
              className="px-3 py-2.5 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider w-[25%]"
            >
              이슈
            </th>
            <th
              scope="col"
              className="px-3 py-2.5 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider w-[10%]"
            >
              유형
            </th>
            <th
            ...
            <th
              scope="col"
              className="px-3 py-2.5 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider w-[15%]"
            >
              작업
            </th>
          </tr>
          ...
          {issues.map((issue) => (
            ...
            >
              ...
              <td className="px-3 py-3 whitespace-nowrap">
                <span
                  className={`px-2 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full ${getTypeColorById(
                    issue.typeId,
                    projectTypes
                  )}`}
                >
                  {getTypeNameById(issue.typeId, projectTypes)}
                </span>
              </td>
              <td className="px-3 py-3 whitespace-nowrap">
                <span
                  className={`px-2 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusColorById(
                    issue.statusId,
                    projectStatuses
                  )}`}
                >
                  {getStatusNameById(issue.statusId, projectStatuses)}
                </span>
              </td>
              ...
              <td className="px-3 py-3 whitespace-nowrap text-sm font-medium">
                <div className="flex items-center space-x-1">
                  <select
                    value={getStatusNameById(issue.statusId, projectStatuses)}
