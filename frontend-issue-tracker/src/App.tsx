...
const App: React.FC = () => {
  ...
  const baseFilteredIssues = useMemo(() => {
    ...
    if (searchTerm.trim()) {
      ...
      tempIssues = tempIssues.filter(issue =>
        ...
        (issue.comment && issue.comment.toLowerCase().includes(lowerSearchTerm)) ||
        issue.id.toLowerCase().includes(lowerSearchTerm) || // Search by ID
        (issue.affectsVersion && issue.affectsVersion.toLowerCase().includes(lowerSearchTerm)) ||
        (issue.fixVersion && issue.fixVersion.toLowerCase().includes(lowerSearchTerm))
      );
    }
    return tempIssues;
  ...
  const handleAddIssue = useCallback(async (formData: IssueFormData) => {
    setError(null);
    setIsSubmitting(true);
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ message: '이슈 추가에 실패했습니다.' }));
        throw new Error(errorData.message || `이슈 추가에 실패했습니다: ${response.statusText}`);
      }
      await fetchIssues(); // Refetch all issues
      setShowAddIssueModal(false);
    } catch (err) {
      console.error("이슈 추가 중 오류:", err);
      setError((err as Error).message || "이슈 추가 중 알 수 없는 오류가 발생했습니다.");
    } finally {
      setIsSubmitting(false);
    }
  }, [fetchIssues]);
  ...
  const handleEditIssue = useCallback(async (issueId: string, formData: IssueFormData) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ message: '이슈 수정에 실패했습니다.' }));
        throw new Error(errorData.message || `이슈 수정에 실패했습니다: ${response.statusText}`);
      }
      const updatedIssueFromServer: Issue = await response.json();
      setIssues(prevIssues => prevIssues.map(issue =>
        issue.id === updatedIssueFromServer.id ? updatedIssueFromServer : issue
      ));
      ...
      setError((err as Error).message || "이슈 수정 중 알 수 없는 오류가 발생했습니다.");
    } finally {
      setIsSubmitting(false);
    }
  }, [fetchIssues, selectedIssueForDetail]);

  const updateIssueStatus = useCallback(async (issueId: string, newStatus: StatusEnum) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ message: '이슈 상태 업데이트에 실패했습니다.' }));
        throw new Error(errorData.message || `이슈 상태 업데이트에 실패했습니다: ${response.statusText}`);
      }
      const updatedIssueFromServer: Issue = await response.json();
      setIssues(prevIssues => prevIssues.map(issue =>
        issue.id === updatedIssueFromServer.id ? updatedIssueFromServer : issue
      ...
      setError((err as Error).message || "이슈 상태 업데이트 중 알 수 없는 오류가 발생했습니다.");
  ...
  const confirmDeleteIssue = useCallback(async () => {
    if (issueToDelete) {
      ...
      try {
        const response = await fetch(`/api/issues/${issueToDelete}`, { method: 'DELETE' });
        if (!response.ok && response.status !== 204) {
          const errorData = await response.json().catch(() => ({ message: '이슈 삭제에 실패했습니다.' }));
          throw new Error(errorData.message || `이슈 삭제에 실패했습니다: ${response.statusText}`);
        }
        ...
        setError((err as Error).message || "이슈 삭제 중 알 수 없는 오류가 발생했습니다.");
      } finally {
  ...
  if (isLoading && issues.length === 0) {
    return (
      <div className="flex h-screen items-center justify-center bg-slate-100">
        <div className="text-2xl font-semibold text-slate-700">이슈 로딩 중...</div>
      </div>
    );
  }
    ...
    <div className="flex h-screen bg-white text-slate-800 font-sans">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-x-auto overflow-y-auto bg-slate-50 p-4 sm:p-6">
          {error && (
            <div className="mb-4 p-3 bg-red-100 border border-red-300 text-red-700 rounded-md shadow-sm" role="alert">
              <div className="flex justify-between items-center">
                <div>
                  <p className="font-bold">오류:</p>
                  <p className="text-sm">{error}</p>
                </div>
                <button onClick={() => setError(null)} className="text-red-600 hover:text-red-800" aria-label="오류 메시지 닫기">
      ...
      <Modal isOpen={showAddIssueModal} onClose={() => { setShowAddIssueModal(false); setError(null);}} title="새 이슈 생성">
        <IssueForm
      ...
      {selectedIssueForEdit && (
        <Modal isOpen={showEditIssueModal} onClose={handleCloseEditModal} title="이슈 수정">
          <IssueForm
            onSubmit={(formData) => handleEditIssue(selectedIssueForEdit.id, formData)}
      ...
      {showDeleteModal && issueToDelete && (
        <ConfirmationModal
          title="삭제 확인"
          message={`정말로 이슈 "${issues.find(i => i.id === issueToDelete)?.content.substring(0,30) ?? '선택된 이슈'}..."을(를) 삭제하시겠습니까?`}
